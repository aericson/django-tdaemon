...
class TestDjangoTDaemon(TestCase):
    ...
    def _create_app(cls, app, number_of_tests):
        app_path = os.path.join(cls.dir.path, app)
        os.mkdir(app_path)
        sub_call(
    ...
    def setUpClass(cls):
        ...
        cls.dir = TempDirectory()
        sub_call(
            ['django-admin', 'startproject', 'test_proj',
             cls.dir.path])
        cls.proj_path = os.path.join(cls.dir.path, 'test_proj')
        sys.path.append(cls.dir.path)
    ...
    def test_watcher_all(self, apps_mock, all_mock):
        watcher = Watcher(self.dir.path, self.settings)
        watcher.start()
        # import pdb; pdb.set_trace()
        self.touch(os.path.join(self.proj_path, 'settings.py'))
        time.sleep(1)
        watcher.stop()
        all_mock.assert_called_once_with(self.dir.path)
    ...
    def test_watcher_lonly(self, apps_mock, all_mock):
        watcher = Watcher(self.dir.path, self.settings)
        watcher.start()
        self.touch(os.path.join(self.dir.path, 'app1', 'admin.py'))
        time.sleep(1)
        watcher.stop()
        self.assertFalse(all_mock.called)
        apps_mock.assert_called_once_with(self.dir.path, ['app1'])
    ...
    def test_watcher_with_more_apps(self, apps_mock, all_mock):
        ...
        watcher.pause()
        self.touch(os.path.join(self.dir.path, 'app1', 'admin.py'))
        self.touch(os.path.join(self.dir.path, 'app2', 'admin.py'))
        watcher.pause()
        time.sleep(1)
        watcher.stop()
        self.assertFalse(all_mock.called)
        ...
        app_list = ['app1', 'app2']
        a1 = call(self.dir.path, app_list) == apps_mock.call_args
        a2 = call(self.dir.path, app_list[::-1]) == apps_mock.call_args
    ...
    def test_event_on_create_py(self, test_mock):
        ...
        test_mock.assert_called_with(
            [os.path.join(self.dir.path, 'app1', 'admin.py')]
        )
    ...
    def test_event_on_create_pyc(self, test_mock):
        watcher = Watcher(self.dir.path, self.settings)
        watcher.start()
        self.touch(os.path.join(self.dir.path, 'app1', 'admin.pyc'))
        time.sleep(1)
        watcher.stop()
    ...
    def test_event_on_create_tmp(self, test_mock):
        watcher = Watcher(self.dir.path, self.settings)
        watcher.start()
        self.touch(os.path.join(self.dir.path, 'app1', 'admin.tmp'))
        time.sleep(1)
        watcher.stop()
        self.assertFalse(test_mock.called)
    ...
    def test_event_on_mv_tmp(self, test_mock):
        watcher = Watcher(self.dir.path, self.settings)
        watcher.start()
        self.touch(os.path.join(self.dir.path, 'app1', 'module.tmp'))
        os.rename(os.path.join(self.dir.path, 'app1', 'module.tmp'),
                  os.path.join(self.dir.path, 'app1', 'module.py'))
        time.sleep(1)
        watcher.stop()
        test_mock.assert_called_with(
            [os.path.join(self.dir.path, 'app1', 'module.py')]
    ...
    def test_create_and_mv_folder(self, test_mock):
        watcher = Watcher(self.dir.path, self.settings)
        watcher.start()
        os.mkdir(os.path.join(self.dir.path, 'app1', 'a_folder'))
        time.sleep(1)
        self.assertFalse(test_mock.called)
        open(os.path.join(self.dir.path, 'app1', 'a_folder', 'file.py'),
             "w").close()
        time.sleep(1)
        test_mock.assert_called_with(
            [os.path.join(self.dir.path, 'app1', 'a_folder', 'file.py')]
        )
        os.rename(os.path.join(self.dir.path, 'app1', 'a_folder'),
                  os.path.join(self.dir.path, 'app1', 'a_folder1'))
        time.sleep(2)
        test_mock.assert_called_with(
            [os.path.join(self.dir.path, 'app1', 'a_folder', 'file.py'),
             os.path.join(self.dir.path, 'app1', 'a_folder1', 'file.py')
             ]
        )
        watcher.stop()
